...
// pt[0] is args passed as inputs; pt[1] is output pointer
  ...
  n = B->nx*B->ny;
  char *hmsg[]=  {
    "Kit (Kulina edITor) is supposed to be simple graphical",
    "editor for Linux. It is aimed as an editor which be can be",
    "used with few trials. However few tips may be useful.",
    "",
    "1. Markpos",
    "This position is used by operations like ,'write to', 'cut'",
    "and 'copy'. It is used as the start position for these commands.",
    "Also the 'Go' button (int the lower bar) can take the cursor",
    "to 'Markpos'. Markpos can be set either by pressing the 'Mark'",
    "button in the top bar or by typing explicitly at the",
    "'MarkPos' text box in bottom bar.",
    "",
    "2. Search and Replace strings.",
    "These strings are set at the respective text boxes in top",
    "bar. Either of the strings can be empty. Search string",
    "is used for searching the pattern. Replace will",
    "replace the next search string with the replace string.",
    "If the search string is empty, a 'replace' will insert the",
    "replace string at the current cursor position.",
    "If the replace string is empty , a 'replace' will remove the",
    "next search string.",
    "",
    "Note: Text input box  can be cleared by typing an 'Esc' character.",
   NULL};
  switch(butno) {
    case 1: //  Help 
      RunMsg(Tmp,hmsg);
