...
static Dlink *ModuleList=NULL;
 ...
 /* Callback for  Msgscroll1   */ 

void * WmsgMsgscroll1init (DIS *S,void *ptmp) {
...
void  WmsgSplbutn1init (DIL *B,void *ptmp) {
 void **pt=(void **)ptmp; //pt[0] is arg 
...
void *  WmsgAction(void *Tmp,void *Args) {
  return NULL;
} 
 
 
int   WmsgOn(void *itmp) {
...
void * WmsgInterface(void *args,void *rets) {
  /*********************************** 
   ***********************************/ 
  ...
  it->ysh=0;
  it->RunDia = RunWmsg;
  it->MakeGroup = MakeWmsgGroup;
  it->Title = GetPointer((char *)"Wmsg");
  it->Help = GetPointer( (char *)"No help yet, request");
  it->Action = WmsgAction;
  it->Settings = WmsgSetup;
  it->Cleanup  = WmsgCleanDia;
  if(args != NULL) Args=args;
  if(rets != NULL) Rets=rets;
  it->args = Args;
  it->rets = Rets;
  it->SwitchOn = WmsgOn;
  it->SwitchOff = WmsgOff;
...
void  WmsgSplbutn2init (DIL *B,void *ptmp) {
 void **pt=(void **)ptmp; //pt[0] is arg 
// may use kgChangeButtonNormalImage etc...
 BUT_STR *buts;
 buts = (BUT_STR *) (B->buts);
...
// pt[0] is args passed as inputs; pt[1] is output pointer
 /* pt[0] is inputs, given by caller */
  void **Args=(void **)pt[0];
  char *cpt,*Msg;
  int *Sbutn = (int *) Args[2];
  if( *Sbutn == 0) {
     DIL *B = (DIL *)kgGetNamedWidget(Tmp,(char *)"Splbutn1");
     kgSetWidgetVisibility(B,1);
     kgUpdateWidget(B);
  }
  else {
     DIL *B = (DIL *)kgGetNamedWidget(Tmp,(char *)"Splbutn2");
     kgSetWidgetVisibility(B,1);
     kgUpdateWidget(B);
  }
  DII *I= (DII *)kgGetNamedWidget(Tmp,(char *)"Message1");
  kgWrite(I,(char *)Args[1]);
  DIS *S = (DIS *)kgGetNamedWidget(Tmp,(char *)"Msgscroll1");
  Dlink *L = (Dlink *)Args[0];
  Resetlink(L);
  while ( (cpt = (char *)Getrecord(L))!= NULL) kgWrite(S,cpt);
//  kgUpdateOn(Tmp);;
  return ret;
}
